...
struct AccessSettingsView: View {
    private static let groqAPIKeyPattern = #"^gsk_[A-Za-z0-9]{40,}$"#
    ...
    @State private var testErrorText: String?
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack(alignment: .top, spacing: 8) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        ...
                        Text("Öffne Bedienungshilfen und aktiviere QuickDictate. Falls QuickDictate schon aktiv ist, einmal aus- und wieder einschalten.")
            ...
            VStack(alignment: .leading, spacing: 8) {
                HStack {
                    ...
                    Spacer()
                    if appState.hasValue(for: .groqAPIKey) && !editingAPIKey {
                ...
                Text("Dein Key bleibt lokal in dieser App. Audio und Text werden direkt an die Groq API gesendet.")
                    .font(.system(size: 10.5))
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                if !currentInstallLocation.isCanonicalInstall {
                    Text("Hotkeys und Login-Start laufen am stabilsten, wenn QuickDictate aus /Applications gestartet wird.")
                        .font(.system(size: 10.5))
                        .foregroundStyle(.orange)
                        .fixedSize(horizontal: false, vertical: true)
                } else {
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text("Für direktes Einfügen: QuickDictate einmal nach /Applications legen und danach Mikrofon sowie Bedienungshilfen erlauben.")
                    .font(.system(size: 11))
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
                    .padding(10)
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("Vor dem Löschen QuickDictate erst auf diesem Mac bereinigen. So verschwinden Anmeldestart und lokale Daten sauber aus dem Weg.")
                    .font(.system(size: 10.5))
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
                ...
                if showCleanupOptions {
                    ...
                    Text("Danach QuickDictate beenden und die App aus /Applications löschen. Bereits verwaiste alte Login-Items können in den Systemeinstellungen einmalig manuell entfernt werden.")
                        .font(.system(size: 10.5))
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
        ...
        .onAppear {
            ...
            refreshInstallState()
            load()
            if !appState.hasValue(for: .groqAPIKey) {
                editingAPIKey = true
                focusedField = .groqAPIKey
            }
        }
    }
    ...
    private func save() {
        ...
        if editingAPIKey || !appState.hasValue(for: .groqAPIKey) {
            guard !trimmedAPIKey.isEmpty else {
                saveErrorText = "Bitte trage deinen Groq API Key ein."
                return
            }
            do {
                try KeychainService.save(key: .groqAPIKey, value: trimmedAPIKey)
                groqAPIKey = ""
                editingAPIKey = false
            } catch {
        ...
        KeychainService.invalidateCache()
        if !appState.hasValue(for: .groqAPIKey) {
            saveErrorText = "Groq API Key wurde nicht persistent gespeichert. Bitte App neu starten und erneut versuchen."
            return
        }
    ...
    private func pasteAPIKeyFromClipboard() {
        ...
        guard trimmedKey.range(of: Self.groqAPIKeyPattern, options: .regularExpression) != nil else {
            saveErrorText = "Zwischenablage enthält keinen plausiblen Groq API Key (erwartet: gsk_...)."
            return
        }
    ...
    private var installationHeadline: String {
        ...
        case .applications:
            return "QuickDictate liegt am richtigen Ort."
        case .userApplications:
            return "QuickDictate liegt noch in ~/Applications."
        case .outsideApplications:
            return "QuickDictate liegt noch nicht in /Applications."
        case .unknown:
    ...
    private var installationDetail: String {
        ...
        case .userApplications:
            return "Für stabile Hotkeys und Login-Items sollte QuickDictate nur aus /Applications laufen."
        case .outsideApplications:
            return "Verschiebe QuickDictate einmal nach /Applications, damit Anmeldestart und Hotkeys sauber bleiben."
        case .unknown:
            return "Öffne QuickDictate möglichst direkt aus /Applications."
        }
    }
    ...
    private func runCleanup() {
        ...
        if report.failedItems.isEmpty {
            cleanupStatusText = deleteLocalDataOnCleanup
                ? "Anmeldestart und lokale Daten wurden bereinigt. Jetzt QuickDictate beenden und aus /Applications löschen."
                : "Anmeldestart wurde deaktiviert. Jetzt QuickDictate beenden und aus /Applications löschen."
            showCleanupOptions = false
    ...
    private func testGroqConnection() {
        ...
        Task {
            do {
                try await GroqLLMService.testConnection(model: appState.appSettings.groqChatModel)
                await MainActor.run {
                    isTesting = false
                    testStatusText = "Verbindung erfolgreich."
                    testErrorText = nil
                }
            } catch {
                await MainActor.run {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                SectionLabel(text: "Groq Modelle")
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Picker("", selection: Binding(
                        get: { appState.appSettings.groqTranscriptionModel },
                        set: { appState.appSettings.groqTranscriptionModel = $0 }
                    )) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Picker("", selection: Binding(
                        get: { appState.appSettings.groqChatModel },
                        set: { appState.appSettings.groqChatModel = $0 }
                    )) {
                        ForEach(GroqChatModel.allCases) { model in
                            Text(model.displayName).tag(model.rawValue)
                        }
                    }
                    .labelsHidden()
                ...
                Text("Modelle werden sofort übernommen. Keine Neustart nötig.")
                    .font(.system(size: 10.5))
                    .foregroundStyle(.secondary)
            }
