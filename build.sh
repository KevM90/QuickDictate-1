...
# QuickDictate macOS App - Build & Run
...
# App finden
APP_PATH="$DERIVED_DATA_PATH/Build/Products/$BUILD_CONFIGURATION/QuickDictate.app"
...
DEST="$SCRIPT_DIR/QuickDictate.app"
rm -rf "$DEST"
cp -R "$APP_PATH" "$DEST"
...
if [ "$INSTALL_APP" = true ]; then
    ...
    INSTALL_DEST="$APPS_DIR/QuickDictate.app"
    if [ ! -w "$APPS_DIR" ]; then
...
echo "4. In QuickDictate deinen eigenen Groq API Key eintragen"
echo "5. Loslegen und bei Bedarf im Code weiterbauen"
echo ""

# Optional: direkt starten
if [ "$RUN_AFTER" = true ]; then
    echo "🚀 Starte QuickDictate ..."
    open "$RUN_TARGET"
fi
